...
export interface LocalEntry {
  ...
  /** Matching sidecar JPEG in the source folder (moved with the RAW). */
...
export class LocalSource implements PhotoSource {
  ...
  private async moveEntry(id: string, dest: FileSystemDirectoryHandle): Promise<void> {
    ...
    if (entry.sidecarHandle && entry.sidecarName) {
      try {
        ...
        // Sidecar may have been deleted manually; RAW move already succeeded.
      }
    }
  }
